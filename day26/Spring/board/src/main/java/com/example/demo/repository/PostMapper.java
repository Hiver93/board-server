...
public interface PostMapper {
	...
	@Insert("INSERT INTO post(title, content, password, view, created_at, updated_at) "
			+ "VALUES (#{post.title}, #{post.content}, #{post.password}, #{post.view}, NOW(), NOW())")
	...
	@Update("UPDATE post SET title = #{post.title}, content = #{post.content}, updated_at = now() WHERE id = #{post.id}")
	public void updateTitleAndContent(@Param("post") Post post);
	
	@Update("UPDATE post SET view = #{post.view} WHERE id = #{post.id}")
	public void updateView(@Param("post")Post post);
}
