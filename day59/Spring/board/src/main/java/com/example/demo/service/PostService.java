...
public class PostService {
	...
	public List<Post> getPostList(Pageable pageable, String keyword, Set<String> target){
		...
		boolean content = target.contains("contain");
	...
	public void removePost(Integer postId, User user) {
		...
		if(!saved.getUser().getId().equals(user.getId())) {
			throw new BoardException(ErrorCode.NOT_POST_ONWER);
		}
	...
	@Transactional
	public void addComment(String content, Integer postId, User user) {
		Post saved = this.postRepository.findById(postId).orElseThrow(()->{throw new BoardException(ErrorCode.POST_NOT_FOUND);});
	...
	public void removePostLike(Integer postId, User user) {
		PostLike saved = this.postLikeRepository.findByUserIdAndPostId(user.getId(), postId).orElseThrow(()->{throw new BoardException(ErrorCode.POST_LIKE_NOT_FOUND);});
