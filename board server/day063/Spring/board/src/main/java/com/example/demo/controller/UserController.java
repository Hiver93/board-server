...
public class UserController {
	...
	@PatchMapping("/{userId}")
	public ResponseEntity<BaseResBody<Void>> patchUser(@PathVariable(name="userId") Integer userId, @RequestBody UserReqDto.Patch dto){
		this.userService.modifyNickname(userId, dto.getNickname(), this.authService.authenticate());
		return new BaseResBody<Void>(null, "modified user info")
				.toResponse(HttpStatus.OK);
	}
	
	@DeleteMapping("/{userId}")
	public ResponseEntity<BaseResBody<Void>> deleteUser(@PathVariable(name="userId") Integer userId){
		this.userService.removeUser(userId, this.authService.authenticate());
