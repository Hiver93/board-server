...
import com.example.demo.repository.PostLikeReposiory;
...
public class PostService {
	...
	private PostLikeReposiory postLikeReposiory;
	public PostService(PostRepository postRepository, PostLikeReposiory postLikeReposiory) {
		super();
		this.postRepository = postRepository;
		this.postLikeReposiory = postLikeReposiory;
	}
	...
	public void addLike(Integer postId, User user) {
		Post saved = this.postRepository.findById(postId).orElseThrow(()->{throw new BoardException(ErrorCode.POST_NOT_FOUND);});
		if(this.postLikeReposiory.existsByUserIdAndPostId(user.getId(), postId)) {
			throw new BoardException(ErrorCode.POST_LIKE_CONFLICT);
		}
		saved.addLike(PostLike.builder()
				.User(user)
				.post(saved)
				.build());
	}
