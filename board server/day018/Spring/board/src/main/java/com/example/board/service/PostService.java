...
import com.example.board.exceptions.PostException;
...
public class PostService {
	...
	public Post getPost(long postId){
		return this.postRepository.findById(postId).orElseThrow(()->new PostException(HttpStatus.NOT_FOUND, "post not found"));
	}
	
	public void deletePost(long postId, String password) {
		Post saved = this.postRepository.findById(postId).orElseThrow(()->new PostException(HttpStatus.NOT_FOUND, "post not found"));
		if(!saved.getPasswrod().equals(password)) {
			throw new PostException(HttpStatus.UNAUTHORIZED, "password incorrect");
		}
		this.postRepository.delete(saved);
	}
	
	public void modifyPost(Post post) {
		Post saved = this.postRepository.findById(post.getId()).orElseThrow(()->new PostException(HttpStatus.NOT_FOUND, "post not found"));
		if(!saved.getPasswrod().equals(post.getPasswrod())) {
			throw new PostException(HttpStatus.UNAUTHORIZED, "password incorrect");
		}
