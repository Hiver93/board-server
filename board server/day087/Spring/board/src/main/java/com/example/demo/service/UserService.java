...
public class UserService {
	...
	public User getUser(String username, String password) {
		User saved = this.userRepository.findByUsername(username).orElseThrow(()->{throw new BoardException(ErrorCode.INVALI_CREDENTIALS);});
		if(!passwordEncoder.matches(password, saved.getPassword())) {
			throw new BoardException(ErrorCode.INVALI_CREDENTIALS);
		}
		return saved;
	}
	...
	@Transactional
	@Scheduled(cron = "0 0 * * * *")
	public void deleteInactivatedUser() {
		this.userRepository.deleteInactivatedUserOlderThan(LocalDateTime.now().minusHours(1));
	}
}
