...
public interface PostRepository extends JpaRepository<Post, Integer>{
	...
	@Query(value = "SELECT p FROM Post p LEFT JOIN FETCH p.user WHERE p.user.nickname LIKE %:nickname% AND p.user.deletedAt = null",
			countQuery = "SELECT COUNT(*) FROM Post p WHERE p.user.nickname LIKE %:nickname% AND p.user.deletedAt = null")
