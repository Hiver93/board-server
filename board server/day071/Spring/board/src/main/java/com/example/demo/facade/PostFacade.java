...
public class PostFacade {
	...
	public void createPost(PostReqDto.Create dto) {
		User user = this.authService.getAuthenticateion();
		if(user == null) {
	...
	public PostResDto.Detail readPost(Integer postId){
		var detail = PostResDto.Detail.from(this.postService.getPost(postId));
	...
	public void removePost(Integer postId, PostReqDto.Delete dto) {
		...
		if(saved.isAnonymousPost()) {
			...
			this.postService.removePost(postId, this.authService.getAuthenticateion());
		}
	}
	...
	public void modifyPost(Integer postId, PostReqDto.Put dto) {
		...
		if(saved.isAnonymousPost()) {
			...
			this.authService.authenticate();
			this.postService.modifyPost(postId, dto.getTitle(), dto.getContent(), this.authService.getAuthenticateion());
		}
	}
	
	public void addComment(Integer postId, CommentReqDto.Create dto) {
		this.authService.authenticate();
		this.postService.addComment(postId, dto.getContent(), this.authService.getAuthenticateion());
	}
	
	public void addPostLike(Integer postId) {
		this.authService.authenticate();
		this.postService.addPostLike(postId, this.authService.getAuthenticateion());
	}
	
	public void removePostLike(Integer postId) {
		this.authService.authenticate();
		this.postService.removePostLike(postId, this.authService.getAuthenticateion());
	}
}
