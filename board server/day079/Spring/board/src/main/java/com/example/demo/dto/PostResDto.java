	...
	public static record Detail(
		...
		public static Detail from(Post post) {
			...
			if(post.isAnonymousPost()) {
				user = User.builder()
						.id(null).nickname("anonymous").build();
			}
			return new Detail(post.getId(), post.getTitle(), post.getContent() ,post.getView(), post.getComments(), post.getImage().getFileName(), post.getPostLikes(),
