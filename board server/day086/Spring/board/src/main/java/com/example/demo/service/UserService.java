...
public class UserService {
	...
	public User createUser(String username, String password, String nickname) {
		this.userRepository.findByUsername(username).ifPresent(u->{throw new BoardException(ErrorCode.USERNAMEE_CONFLICT);});
	...
	public User getUser(String username, String password) {
		...
		if(!saved.isActivated() || !passwordEncoder.matches(password, saved.getPassword())) {
	...
	public void removeInactivatedUser() {
		this.userRepository.deleteInactivatedUserOlderThan(LocalDateTime.now().minusSeconds(30));
	}
}
