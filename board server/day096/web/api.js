...
async function postPost(postData, image){
...
async function putPost(putData, image){
    const formData = new FormData()
    const jsonBlob = new Blob([JSON.stringify(putData)], { type: "application/json" });
    formData.append('post', jsonBlob)
    formData.append('image', image)
    console.log(formData)
    try{
...
async function deleteComment(commentId){
    try{
        const response = await fetch(baseUrl + `/posts/${0}/comments/${commentId}`,{
            method:"DELETE",
            credentials:"include",
            headers:{
                // "Authorization":localStorage.getItem('jwt')
...
async function putComment(commentId, data){
    try{
        const response = await fetch(baseUrl + `/posts/${0}/comments/${commentId}`,{
            method:"PUT",
            credentials:"include",
...
export {baseUrl, getPost, getTotalSize, getPostList, deletePost, 
    postPost, putPost, login, logout, getProfile, reissue, 
    postComment, deleteComment, putComment,
    postPostLike, deletePostLike,
