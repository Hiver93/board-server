...
public class UserService {
	...
	public void createUser(String username, String password, String nickname) {
		this.userRepository.findByUsername(username).ifPresent(u->{throw new BoardException(ErrorCode.USERNAME_CONFLIICT);});
	...
	public User getUser(String username, String password) {
		User saved = this.userRepository.findByUsername(username).orElseThrow(()->{throw new BoardException(ErrorCode.INVALID_CREDENTAILS);});
		if(!passwordEncoder.matches(password, saved.getPassword())) {
			throw new BoardException(ErrorCode.INVALID_CREDENTAILS);
		}
		return saved;
	}
