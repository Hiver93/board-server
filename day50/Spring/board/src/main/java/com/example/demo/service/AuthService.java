...
public class AuthService {
	...
	public User autheticate() {
		HttpServletRequest request = ((ServletRequestAttributes)RequestContextHolder.getRequestAttributes()).getRequest();
		String token = request.getHeader("Authorization");
		if(token == null) {
			throw new BoardException(ErrorCode.AUTNETICATION_REQUIRED);
		}
		try {
	...
	public void refreshAuthentication() {
		...
		String token = Stream.of(request.getCookies())
				...
				.findAny().orElseThrow(()->{throw new BoardException(ErrorCode.AUTNETICATION_REQUIRED);})
				.getValue();
		
		try {
			this.setAuthentication(this.jwtUtil.getUser(token));
		}catch(ExpiredJwtException e) {
			throw new BoardException(ErrorCode.EXPIRED_JWT);
		}catch(JwtException e) {
			throw new BoardException(ErrorCode.INVALID_JWT);
