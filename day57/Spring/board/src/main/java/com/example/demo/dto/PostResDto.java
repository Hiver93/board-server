...
public class PostResDto {
	...
	public static record Detail(
		...
		public static Detail from(Post post) {
			return new Detail(post.getId(), post.getTitle(), post.getContent(), post.getView(), post.getUser().getId(), post.getUser().getNickname(), post.getComments(),
	...
	public static record PostList(
		...
		private static record PostItem(
			...
			private static PostItem from(Post post) {
				return new PostItem(post.getId(), post.getTitle(), post.getView(), post.getUser().getId(), post.getUser().getNickname(), post.getComments(),
						post.getCreatedAt().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")),
						post.getUpdatedAt().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"))						
						);
			}
