...
public class PostLikeService {
	...
	public PostLike createPostLike(Post post, User user) {
		...
		PostLike saved = this.postLikeRepository.save(PostLike.builder()
						.post(post)
						.user(user)
						.build());
		post.addPostLike(saved);
		return saved;
	}
	
	public void removePostLike(Integer postId, User user) {
		PostLike saved = this.postLikeRepository.findById(postId).orElseThrow(()->{throw new BoardException(ErrorCode.LIKE_NOT_FOUND);});
